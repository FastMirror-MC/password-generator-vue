...
<template>
    <div class="range-input-wrapper">
        <div class="slider-wrapper">
            ...
            }" :class="{ active: isListening }" role="slider" :aria-valuemin="min" :aria-valuemax="max" :aria-valuenow="value" :aria-labelledby="ariaLabelledby" tabindex="0" @pointerdown="onPointerDown" @keydown.left.prevent="value = Math.max(min, value - 1)" @keydown.down.prevent="value = Math.max(min, value - 1)" @keydown.right.prevent="value = Math.min(max, value + 1)" @keydown.up.prevent="value = Math.min(max, value + 1)">
